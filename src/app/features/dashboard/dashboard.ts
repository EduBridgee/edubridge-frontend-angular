...
export class DashboardComponent implements OnInit {
  ...
  fetchData() {
    ...
    const studentsReq = this.http.get<any[]>(`${API_BASE_URL}/students`)
      .pipe(catchError(() => of([])));

    const coursesReq = isDocente
      ? this.http.get<any[]>(`${API_BASE_URL}/courses`).pipe(catchError(() => of([])))
      : of([]);

    const teachersReq = isDocente
      ? this.http.get<any[]>(`${API_BASE_URL}/teachers`).pipe(catchError(() => of([])))
      : of([]);

    const enrollmentsReq = isDocente
      ? this.http.get<any[]>(`${API_BASE_URL}/enrollments`).pipe(catchError(() => of([])))
      : this.http.get<any[]>(`${API_BASE_URL}/enrollments/student/${userId}`).pipe(catchError(() => of([])));

    const gradesReq = isDocente
      ? this.http.get<any[]>(`${API_BASE_URL}/grades`).pipe(catchError(() => of([])))
  ...
  private processAdminDashboard() {
    ...
    forkJoin({
      allStudents: this.http.get<any[]>(`${API_BASE_URL}/students`).pipe(catchError(() => of([]))),
      allCourses: this.http.get<any[]>(`${API_BASE_URL}/courses`).pipe(catchError(() => of([])))
    }).subscribe({
      next: (res) => {
        this.students = res.allStudents;
